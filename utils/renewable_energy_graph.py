...
def create_renewable_graph(data: pd.Dataframe):
    ...
    for ax, [min, max] in zip(axs, min_max_years):
            ...
            ax.axhline(100000, xmin=renewable_xmin, xmax=renewable_xmin + 0.2 * relation,
                    lw=7, color=renewable_color)
            ax.axhline(100000, xmin=renewable_xmin, xmax=renewable_xmax, lw=7,
                    color=renewable_color, alpha=0.3)
            x_pos = 1935 if round(relation*100) < 10 else 1933
            ax.text(x_pos, 110000, f'{round(relation*100)}%',
                    size=20, color=renewable_color, weight='bold')
            ax.text(1932, 85000, f'Renew. Energy',
    ...
    axs[0].text(1918,235000, "Popularization of sustainable energy in the last 25 years",
            size=17, weight='bold')
    axs[0].text(1918,215000,"World energy consumption for the past 100 years in TWh for each 25 years")
    ...
    plt.savefig('renewable_energy_graph.py')
    plt.show()